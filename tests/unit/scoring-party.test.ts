...
describe('scorepack — prix par personne (fix 28/05)', () => {
  ...
  it('1 voyageur : prix/pers = totalPrice (pas de division)', () => {
    ...
    const score1 = scorepack(pack1, 'student', 1, 'Lisbonne');
    const score4 = scorepack(pack4, 'student', 4, 'Lisbonne');
  ...
  it('groupe de 5 : 35 000€ total → 7 000€/pers → score prix positif', () => {
    ...
    const score = scorepack(pack, 'party', 5, 'Ibiza');
  ...
  it('score party sans events : ne doit PAS être 0', () => {
    ...
    const score = scorepack(pack, 'party', 2, 'Ibiza');
  ...
  it('plus il y a d\'activités nightlife, meilleur le score party', () => {
    ...
    const scoreBase = scorepack(packBase, 'party', 2, 'Ibiza');
    const scoreRich = scorepack(packRich, 'party', 2, 'Ibiza');
  ...
  it('activités non-nightlife n\'améliorent pas le score party', () => {
    ...
    const scoreMusee = scorepack(packMusee, 'party', 2, 'Paris');
    const scoreNight = scorepack(packNight, 'party', 2, 'Ibiza');
...
// ============================================================
describe('scorepack — régression modes non-party', () => {
  ...
  it('mode luxury : les activités nightlife ne boostent pas le score', () => {
    ...
    const scoreNight = scorepack(packLuxe,     'luxury', 2, 'Monaco');
    const scoreLuxe  = scorepack(packLuxeVrai, 'luxury', 2, 'Monaco');
  ...
  it('mode student : prix bas reste la priorité absolue', () => {
    ...
    const scoreCheap = scorepack(packCheap, 'student', 1, 'Prague');
    const scoreCheer = scorepack(packCheer, 'student', 1, 'Prague');
