...
const schemas = {
  ...
  Trip: {
    ...
    properties: {
      ...
      mode:        { type: 'string', enum: ['party', 'student', 'luxury', 'group', 'relax', 'surprise'] },
  ...
  Preferences: {
    ...
    properties: {
      user_id:         { type: 'string', format: 'uuid' },
      default_mode:    { type: 'string', enum: ['party', 'student', 'luxury', 'group', 'relax', 'surprise'] },
  ...
  paths: {
            ...
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  mode:        { type: 'string', enum: ['party', 'student', 'luxury', 'group', 'relax', 'surprise'], example: 'relax' },
                },
              },
            },
          },
    ...
    '/api/ai/destinations': {
      post: {
        ...
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['mode'], properties: { mode: { type: 'string', enum: ['party', 'student', 'luxury', 'group', 'relax', 'surprise'] }, budget: { type: 'number' }, travelers: { type: 'integer' }, duration: { type: 'integer' }, origin: { type: 'string' }, departure: { type: 'string', format: 'date' } } } } } },
    ...
    '/api/trips': {
      ...
      post: {
        ...
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['destination', 'mode'], properties: { destination: { type: 'string' }, mode: { type: 'string', enum: ['party', 'student', 'luxury', 'group', 'relax', 'surprise'] }, title: { type: 'string' }, country: { type: 'string' }, origin: { type: 'string' }, departure: { type: 'string', format: 'date' }, return_date: { type: 'string', format: 'date' }, travelers: { type: 'integer', minimum: 1, maximum: 50 }, budget: { oneOf: [{ type: 'string' }, { type: 'number' }] }, pack_data: { type: 'object', additionalProperties: true }, score: { type: 'number' } } } } } },
    ...
    '/api/preferences': {
      ...
      put: {
        ...
        security: [{ cookieAuth: [] }, { bearerAuth: [] }],
        requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { default_mode: { type: 'string', enum: ['party', 'student', 'luxury', 'group', 'relax', 'surprise'] }, preferred_prefs: { type: 'array', items: { type: 'string' }, maxItems: 10 }, home_city: { type: 'string' }, currency: { type: 'string', minLength: 3, maxLength: 3, example: 'EUR' } } } } } },
